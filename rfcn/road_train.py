...
def train_net(args, ctx, pretrained, epoch, prefix, begin_epoch, end_epoch, lr, lr_step):
    ...
    image_set = config.dataset.image_set
    roidb = load_gt_roidb(config.dataset.dataset, image_set, config.dataset.root_path, config.dataset.dataset_path)
    ...
    # 100 is max predict bbox, 5 is dimension(x1,y1, x2, y2, cls)
    ...
    if config.TRAIN.RESUME:  # this config.TRAIN.RESUME should be false on this training
    ...
    mod.fit(train_data, eval_metric=eval_metrics, epoch_end_callback=epoch_end_callback,
            ...
            arg_params=arg_params, aux_params=aux_params, begin_epoch=begin_epoch, num_epoch=end_epoch, allow_extra=False)
